...
LOCAL_URL = os.getenv("LOCAL_PING") # Backend de contabilidad - movil local
DEPLOY_URL = os.getenv("PRODU_PING") # Backend de contabilidad - movil deployado
            ...
            try:
                ...
                async with session.get(TARGET_URL, timeout=10) as resp:
                    ...
                    if status_code in [200, 201]:
                        log_message = f"200 ping a {TARGET_URL}|next_ping={delay}s"
                    else:
                        log_message = f"{status_code} Error en {TARGET_URL}|next_ping={delay}s"
