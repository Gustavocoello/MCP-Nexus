...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --scope      Only sync specific scope (root, jarvis, koda, lamar, ragel, nexus, ui, api, database, sdk, mcp_server)"
            exit 0
            ;;
        *)
...
# REPO_ROOT apunta a: backend/src/services/agent/
# WORKSPACE_ROOT apunta a: mcp-scratch/ (Subiendo 3 niveles desde agent/)

get_agents_path() {
    local scope="$1"
    
    # Calculamos la raíz absoluta de tu proyecto (mcp-scratch)
    local WORKSPACE_ROOT="$REPO_ROOT/../../../.."

    case "$scope" in
        # --- Root del Sistema de Agentes ---
        root)       echo "$REPO_ROOT/AGENTS.md" ;;
        
        # --- Agent Domains (Viven en la misma carpeta REPO_ROOT) ---
        jarvis)     echo "$REPO_ROOT/jarvis/AGENT.md" ;;
        koda)       echo "$REPO_ROOT/Koda/AGENT.md" ;;
        lamar)      echo "$REPO_ROOT/lamar/AGENT.md" ;;
        ragel)      echo "$REPO_ROOT/ragel/AGENT.md" ;;
        nexus)      echo "$REPO_ROOT/nexus/AGENT.md" ;;
        ...
        # El SDK está totalmente fuera de mcp-scratch, subimos un nivel más
        sdk)        echo "$WORKSPACE_ROOT/jarvis-sdk/AGENTS.md" ;;
...
echo -e "${BLUE}Processing: $scope -> $(basename "$(dirname "$agents_path")")/$(basename "$agents_path")${NC}"
echo "========================================================"
echo ""
