...
class DynamicAuth(httpx.Auth):
    ...
    def auth_flow(self, request: httpx.Request):
        # Genera el token fresco
        token = self.key_func()
        # Sobrescribe el header de autorización para Vertex AI
...
#=========== CEREBRAS ============
CEREBRAS_API_KEY0 = os.getenv("CEREBRAS_KEY0")
...
API_PROVIDERS = [
    ...
    #========= BLOQUE CEREBRAS =========
    {
        "name": "Cerebras api key 0",
        "client": OpenAI(base_url="https://api.cerebras.ai/v1", 
                         api_key=CEREBRAS_API_KEY0),
        "model": "gpt-oss-120b"
    }    
]
...
API_PROVIDERS_TO_AGENT = [
    # ------ GEMINI DE PAGO (GCP) - eliminar antes del 29-05-2026
    {
        ...
        "model": "google/gemini-3.1-pro-preview" # "google/gemini-3.1-pro-preview" - gemini-1.5-pro-002
    }, 
     # ---- BLOQUE OPENROUTER
    {
...
def get_langchain_llm(temperature: int = 0) -> BaseChatModel:
    """
    Itera API_PROVIDERS_TO_AGENT, construye todos los válidos
    y devuelve una cascada de fallbacks (Plan A → Plan B → Plan C).
    """
    valid_llms = []
    ...
    primary = valid_llms[0]
    fallbacks = valid_llms[1:]

    if fallbacks:
        logger.info(f"LLM Principal: {primary.__class__.__name__}. Configuradas {len(fallbacks)} rutas de escape.")
