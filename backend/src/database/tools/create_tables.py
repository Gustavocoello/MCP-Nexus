...
from src.database.settings.postgres.database_linux_config import get_pg_engine as linux_pg_engine
    ...
    def create_tables(self, db_name, drop_first=False):
        ...
        try:
            ...
            if db_name != 'azure':
                ...
                logger.info("ℹ️ Saltando pgvector en Azure (no compatible/no necesario)")
            # -------------------------------------

            logger.info(f"Creando tablas en {db_name}...")
            
            # Forzamos el uso de la Metadata de nuestro nuevo models.py
            from src.database.models import User, Chat, Message, UserToken, AuthProvider, Document, UserIdentity,  LLMLog, SystemStats, PingLog, PomodoroSession
            from src.database.models.models import Base
            metadata = Base.metadata
            
            # --- NUEVO: SELLO DE ALEMBIC ---
            from alembic.config import Config
            from alembic import command
            import os
            
            # Busca el archivo alembic.ini (ajusta la ruta si es necesario)
            alembic_ini_path = os.path.join(root_dir, 'src', 'database', 'migrations', 'alembic.ini')
            
            if os.path.exists(alembic_ini_path):
                if db_name != 'azure':
                    logger.info(f"Sellando Alembic a 'head' en {db_name}...")
                    alembic_cfg = Config(alembic_ini_path)
                    alembic_cfg.set_main_option("sqlalchemy.url", str(engine.url).replace('%', '%%'))
                    command.stamp(alembic_cfg, "head")
                else:
                    logger.info("Saltando sello de Alembic en Azure (SQL Server no requiere migraciones PG).")
            else:
                logger.warning("No se encontro alembic.ini, la DB quedo sin sello de Alembic.")
