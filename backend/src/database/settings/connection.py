# src/database/settings/connection.py
import os
import time
...
def get_database_url():
    ...
    if env == "prod":
        logger.info("Cascada PROD: Linux -> Windows -> Azure")
        
        # Intentar Linux
        try:
            logger.info("Intentando Linux (principal)")
            return try_connection(linux_pg_engine, "Linux PostgreSQL")
        except Exception as e:
            logger.warning("Linux no disponible, intentando Windows")
        
        # Fallback a Windows
        try:
            logger.info("Intentando Windows (fallback)")
            return try_connection(win_pg_engine, "Windows PostgreSQL")
        except Exception as e:
            logger.warning("Windows no disponible, intentando Azure")
    ...
    elif env == "azure":  # <--- AGREGA ESTO
        logger.info("Modo ADMIN: Forzando conexion solo a Azure")
        return try_connection(get_azure_engine, "Azure SQL")
    else:
        # DEV: Solo Windows
